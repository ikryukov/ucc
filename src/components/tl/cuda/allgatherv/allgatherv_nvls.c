...
void ucc_tl_cuda_allgatherv_nvls_progress(ucc_coll_task_t *coll_task)
{
    ...
    case STAGE_KERNEL:
        ...
        if (ucc_unlikely(status != UCC_OK)) {
            ucc_error("failed to post allgatherv kernel");
    ...
    ucc_ee_h ee, ucc_ev_t *ev, ucc_coll_task_t *coll_task)
{
    ...
    (void)ev; // Mark as intentionally unused
    ...
    ucc_coll_task_t **task_p)
{
    ...
    /* Get offset and count in datatype elements, then convert to bytes and then uint32_t units.
     * Datatype agnostic - we just copy raw bytes.
     * NVLS requires 16-byte alignment (4 uint32_t = 16 bytes). */
    size_t offset_elements   = ucc_tl_cuda_allgatherv_get_offset(task, trank);
    size_t count_elements    = ucc_tl_cuda_allgatherv_get_count(task, trank);
    size_t offset_bytes      = offset_elements * dt_size;
    size_t count_bytes       = count_elements * dt_size;
    ...
    if (task->allgatherv_nvls.offset % 4 != 0) {
        tl_debug(
    ...
    if (task->allgatherv_nvls.count % 4 != 0) {
        tl_debug(
            UCC_TL_TEAM_LIB(team),

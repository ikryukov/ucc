...
ucc_status_t post_allgatherv_kernel(
    ...
    size_t my_count, size_t total_count, CUdeviceptr mc_control_addr,
    CUdeviceptr uc_control_addr, uint64_t launch_counter, uint32_t rank,
    uint32_t tsize, ucc_datatype_t datatype)
{
    ...
    (void)datatype; // Mark as intentionally unused

...
__global__ void __launch_bounds__(UCC_TL_CUDA_MAX_NVLS_THREADS)
    allreduce_kernel_vec32(ucc_tl_cuda_nvls_barrier_t *mc_bar,
                           ucc_tl_cuda_nvls_barrier_t *uc_bar,
                           const uint32_t total_blocks, // block count per gpu * num gpus in Multicast group
                           uint32_t *base_u32, size_t count_u32,
                           uint32_t rank, uint32_t tsize)
{
    (void)total_blocks; // not used in case of per block barrier
    // pre barrier
    nvls_bar(mc_bar, uc_bar, tsize);
                                   ...
                                   uint32_t tsize, ucc_datatype_t datatype)
{
    ...
    ucc_tl_cuda_nvls_barrier_t *mc_bar = reinterpret_cast<ucc_tl_cuda_nvls_barrier_t *>(mc_control_addr);
    ucc_tl_cuda_nvls_barrier_t *uc_bar = reinterpret_cast<ucc_tl_cuda_nvls_barrier_t *>(uc_control_addr);
    ...
    case UCC_DT_FLOAT32:
        ...
        allreduce_kernel_vec32<NvlsFp32Ops><<<sm_count, threads, 0, stream>>>(
            mc_bar, uc_bar, expected_blocks, base_u32, count_u32, rank, tsize);
        break;

...
ucc_status_t ucc_tl_cuda_reduce_scatterv_nvls_start(ucc_coll_task_t *coll_task)
{
    ...
    if (args->coll_type == UCC_COLL_TYPE_REDUCE_SCATTER) {
        if (UCC_IS_INPLACE(*args)) {
            ...
            rbuf = PTR_OFFSET(
                sbuf, task->reduce_scatterv_nvls.offset * sizeof(uint32_t));
            src_size_bytes = args->dst.info.count * UCC_TL_TEAM_SIZE(team) *
                             ucc_dt_size(dt);
        } else {
        ...
        if (UCC_IS_INPLACE(*args)) {
            ...
            rbuf = PTR_OFFSET(
                sbuf, task->reduce_scatterv_nvls.offset * sizeof(uint32_t));
            src_size_bytes = ucc_coll_args_get_total_count(
                                 args,
    ...
    status = post_reduce_scatter_kernel(
        ...
        UCC_TL_TEAM_SIZE(team));
    if (ucc_unlikely(status != UCC_OK)) {
        tl_error(UCC_TASK_LIB(task), "failed to post reduce scatter kernel");
        return status;
    }

...
int ucc_topo_is_single_nvlink_domain(const ucc_topo_t *topo)
{
    ...
    if (!ucc_gpu_fabric_cluster_uuid_is_valid(
        ...
        ucc_debug("nvlink domain check: rank 0 GPU %u has no valid fabric "
                  "cluster UUID", (unsigned)dev);
        return 0;
    }
    ...
    for (i = 1; i < size; i++) {
        ...
        if (memcmp(host->gpus[dev].fabric_cluster_uuid, ref_cluster_uuid,
                   UCC_GPU_FABRIC_CLUSTER_UUID_LEN) != 0) {
            ucc_debug("nvlink domain check: rank %u GPU %u cluster_uuid "
                      "differs from rank 0",
                      (unsigned)i, (unsigned)dev);
            return 0;
        }

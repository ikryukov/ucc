...
static inline int align_count(int c)
{
    return (c + NVLS_COUNT_ALIGN - 1) / NVLS_COUNT_ALIGN * NVLS_COUNT_ALIGN;
}

static void fill_counts_and_displacements(
    int size, int count, int *counts, int *displs)
{
    int bias = count / 2;
    int i;

    /* Align counts for NVLS compatibility */
    bias      = align_count(bias);

    counts[0] = align_count(count - bias);
    displs[0] = 0;
    for (i = 1; i < size - 1; i++) {
        if (i % 2 == 0) {
            counts[i] = align_count(count - bias);
        } else {
            counts[i] = align_count(count + bias);
        }
    ...
    if (size % 2 == 0) {
        counts[size - 1] = align_count(count + bias);
    } else {
        counts[size - 1] = align_count(count);
    }
    ...
    : TestCase(_team, UCC_COLL_TYPE_ALLGATHERV, params)
{
    ...
    if (TEST_SKIP_NONE !=
        skip_reduce(
            test_max_size < (msgsize * size), TEST_SKIP_MEM_LIMIT, team.comm)) {
        return;
    }

    counts = (int *)ucc_malloc(size * sizeof(uint32_t), "counts buf");
    UCC_MALLOC_CHECK(counts);
    displacements = (int *)ucc_malloc(
        size * sizeof(uint32_t), "displacements buf");
...
ucc_status_t TestAllgatherv::check()
{
    ...
    for (i = 0; i < size; i++) {
        init_buffer(
            PTR_OFFSET(check_buf, displacements[i] * ucc_dt_size(dt)),
            counts[i],
            dt,
